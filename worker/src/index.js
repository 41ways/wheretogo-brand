  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if (req.method !== 'POST' || !url.pathname.startsWith('/api/')) {
        if (url.pathname === '/') return new Response('wheretogo-brand', { headers: { 'Content-Type': 'text/plain; charset=utf-8' } });
      ...
      if (url.pathname === '/api/guess') {
        ...
        if (g == null) return json({ error: '없는 브랜드' }, 400, h);
        const now = Date.now();
